...
public:
	...
	public:
		CreateTxMediaHeader(CubixLightComposerPtr composer, 
			...
			composer_(composer), size_(size), data_(data), name_(name), description_(description), type_(type), chain_(chain), prev_(prev), created_(created) {}
		void process(errorFunction);

		static IComposerMethodPtr instance(CubixLightComposerPtr composer, uint64_t size, const std::vector<unsigned char>& data, const std::string& name, const std::string& description, TxMediaHeader::Type type, const uint256& chain, Transaction::UnlinkedOutPtr prev, transactionCreatedFunction created) {
			return std::make_shared<CreateTxMediaHeader>(composer, size, data, name, description, type, chain, prev, created); 
		}
