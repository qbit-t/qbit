...
public:
	...
	public:
		...
		bool pushEntity(const uint256& id, TransactionContextPtr ctx) {
			...
			EntityPtr lEntityByName = persistentEntityStore_->locateEntity(ctx->tx()->entityName());
			if (lEntityByName && lEntityByName->id() != id) { 
...
public:
	TransactionStore(const uint256& chain, ISettingsPtr settings, ITransactionStoreManagerPtr storeManager) : 
		...
		opened_(false),
		space_(new db::DbContainerSpace(settings_->dataPath() + "/" + chain.toHex())),
	...
	void selectUtxoByRawAddressAndAsset(const PKey& /*address*/, const uint256& /*asset*/, std::vector<Transaction::NetworkUnlinkedOut>& /*utxo*/, int /*limit*/);
