...
#ifndef QBIT_TXBUZZER_BLOCK_H
#define QBIT_TXBUZZER_BLOCK_H
...
//
class TxBuzzerBlock: public TxEvent {
public:
	TxBuzzerBlock() { type_ = TX_BUZZER_BLOCK; }
	...
	inline std::string name() { return "buzzer_block"; }
...
typedef std::shared_ptr<TxBuzzerBlock> TxBuzzerBlockPtr;

class TxBuzzerBlockCreator: public TransactionCreator {
public:
	TxBuzzerBlockCreator() {}
	TransactionPtr create() { return std::make_shared<TxBuzzerBlock>(); }

	static TransactionCreatorPtr instance() { return std::make_shared<TxBuzzerBlockCreator>(); }
};
