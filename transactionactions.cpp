...
TransactionAction::Result TxCoinBaseVerifyPush::execute(TransactionContextPtr wrapper, ITransactionStorePtr store, IWalletPtr wallet) {
...
TransactionAction::Result TxSpendVerify::execute(TransactionContextPtr wrapper, ITransactionStorePtr store, IWalletPtr wallet) {
	//
...
TransactionAction::Result TxSpendOutVerify::execute(TransactionContextPtr wrapper, ITransactionStorePtr store, IWalletPtr wallet) {
	//
	// all transaction types _must_ pass through this checker
	if (wrapper->tx()->type() == Transaction::SPEND || wrapper->tx()->type() == Transaction::SPEND_PRIVATE) {
		uint32_t lIdx = 0;
...
TransactionAction::Result TxSpendBalanceVerify::execute(TransactionContextPtr wrapper, ITransactionStorePtr store, IWalletPtr wallet) {
	//
	// all transaction types _must_ pass through this checker
	if (wrapper->tx()->type() == Transaction::SPEND || wrapper->tx()->type() == Transaction::SPEND_PRIVATE) {
