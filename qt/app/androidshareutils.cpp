...
bool AndroidShareUtils::checkMimeTypeView(const QString &mimeType)
{
	...
	jboolean verified = QAndroidJniObject::callStaticMethod<jboolean>("app/buzzer/mobile/ShareUtils",
                                              "checkMimeTypeView",
                                              "(Ljava/lang/String;)Z",
...
void AndroidShareUtils::share(const QString &text, const QUrl &url)
{
	...
	jboolean ok = QAndroidJniObject::callStaticMethod<jboolean>("app/buzzer/mobile/ShareUtils",
                                              "share",
...
void AndroidShareUtils::editFile(const QString &filePath, const QString &title, const QString &mimeType, const int &requestId, const bool &altImpl)
{
    ...
    if(requestId <= 0) {
        ...
        // and want the Result back into 'this' handleActivityResult(...)
        // attention: to test JNI with QAndroidActivityResultReceiver you must comment or rename
		// onActivityResult()  method in Q.java - otherwise you'll get wrong request or result codes
        QtAndroid::startActivity(jniIntent, requestId, this);
    }
}
...
JNIEXPORT void JNICALL
  Java_app_buzzer_mobile_MainActivity_setFileUrlReceived(JNIEnv *env,
                                        jobject obj,
                                        jstring url)
{
...
JNIEXPORT void JNICALL
  Java_app_buzzer_mobile_MainActivity_setFileReceivedAndSaved(JNIEnv *env,
                                        jobject obj,
                                        jstring url)
{
    const char *urlStr = env->GetStringUTFChars(url, NULL);
...
JNIEXPORT bool JNICALL
  Java_app_buzzer_mobile_MainActivity_checkFileExits(JNIEnv *env,
                                        jobject obj,
                                        jstring url)
{
    const char *urlStr = env->GetStringUTFChars(url, NULL);
...
JNIEXPORT void JNICALL
  Java_app_buzzer_mobile_MainActivity_fireActivityResult(JNIEnv *env,
                                        jobject obj,
                                        jint requestCode,
                                        jint resultCode)
{
