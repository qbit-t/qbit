...
Item {
	...
	Rectangle {
		...
		QuarkTextEdit {
			...
			font.pointSize: buzzerApp.isDesktop ? Math.round(buzzerClient.scaleFactor * (defaultFontPointSize + 1) * multiplicator_) : (defaultFontPointSize + 2) * multiplicator_
			//lineHeight: buzzerApp.isDesktop ? (buzzerClient.scaleFactor * 1.1) : lineHeight
			readOnly: true
			selectByMouse: buzzerApp.isDesktop ? true : false
			color: buzzerApp.getColor(buzzerClient.theme, buzzerClient.themeSelector, "Material.foreground")
			selectionColor: buzzerApp.getColor(buzzerClient.theme, buzzerClient.themeSelector, "Material.selected")

			property real multiplicator_: isEmoji ? 3.0 : 1.0
