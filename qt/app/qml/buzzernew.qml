...
QuarkPage
{
	...
	QuarkToolBar {
		...
		QuarkLabel {
			...
			text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.create.new.info.caption")
	...
	QuarkLabel {
		...
		text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.create.new.info")
	}
	...
	Column {
		...
		QuarkRadioButton {
			id: normal
			checked: true
			text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.create.new.normal")
		...
		QuarkRadioButton {
			id: advanced
			text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.create.new.advanced")
			font.pointSize: defaultFontPointSize * buzzerClient.scaleFactor
			width: parent.width
		}
		*/
		QuarkRadioButton {
			id: high
			text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.create.new.high")
			font.pointSize: defaultFontPointSize * buzzerClient.scaleFactor
			width: parent.width
		}
	}
		...
		onClicked: {
			...
			if (normal.checked) {
				...
				if (lComponent.status === Component.Error) {
					...
					lPage = lComponent.createObject(window);
					lPage.controller = controller;
					lPage.initialize(true /*new key*/, true /*activate key*/);
