...
QuarkPage {
	id: buzzfeedmistrusts_
	key: "buzzfeedmistrusts"
	...
	function start(buzzer) {
		//
		eventsThread_ = buzzerClient.createBuzzerMistrustsList();
	...
	BuzzerCommands.LoadMistrustsByBuzzerCommand {
		id: modelLoader
		model: eventsThread_
		...
		header: ItemDelegate {
			...
			QuarkToolBar {
				...
				QuarkLabelRegular {
					...
					text: buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.mistrusters")
					font.pointSize: 18
				}
		...
		delegate: Item {
			...
			*/
			...
			Component.onCompleted: {
				...
				buzzItem.width = list.width;
				buzzItem.controller_ = buzzfeedmistrusts_.controller;
