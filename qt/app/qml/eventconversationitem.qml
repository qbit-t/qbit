...
Item {
	...
	//
	// event (conversation, accept, decline)
	//
	...
	QuarkSymbolLabel {
		...
		function getSymbol() {
			//
			if (type_ === buzzerClient.tx_BUZZER_CONVERSATION()) return Fonts.conversationStartedSym;
			else if (type_ === buzzerClient.tx_BUZZER_ACCEPT_CONVERSATION()) return Fonts.conversationAcceptedSym;
			else if (type_ === buzzerClient.tx_BUZZER_DECLINE_CONVERSATION()) return Fonts.conversationDeclinedSym;
			else if (type_ === buzzerClient.tx_BUZZER_MESSAGE() ||
					 type_ === buzzerClient.tx_BUZZER_MESSAGE_REPLY()) return Fonts.conversationMessageSym;
			return Fonts.questionCircleSym;
		}

		function getColor() {
			//
			if (type_ === buzzerClient.tx_BUZZER_CONVERSATION()) {
				return buzzerApp.getColor(buzzerClient.theme, buzzerClient.themeSelector, "Buzzer.event.conversation");
			} else if (type_ === buzzerClient.tx_BUZZER_ACCEPT_CONVERSATION()) {
				return buzzerApp.getColor(buzzerClient.theme, buzzerClient.themeSelector, "Buzzer.event.conversation.accept");
			} else if (type_ === buzzerClient.tx_BUZZER_DECLINE_CONVERSATION()) {
				return buzzerApp.getColor(buzzerClient.theme, buzzerClient.themeSelector, "Buzzer.event.conversation.decline");
			} else if (type_ === buzzerClient.tx_BUZZER_MESSAGE() ||
	...
	QuarkLabelRegular {
		...
		function getText() {
			//
			if (type_ === buzzerClient.tx_BUZZER_CONVERSATION()) {
				return buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.event.conversation.one");
			} else if (type_ === buzzerClient.tx_BUZZER_ACCEPT_CONVERSATION()) {
				return buzzerApp.getLocalization(buzzerClient.locale, "Buzzer.event.conversation.accepted.one");
			} else if (type_ === buzzerClient.tx_BUZZER_DECLINE_CONVERSATION()) {
