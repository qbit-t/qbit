...
QuarkPage {
	id: buzzfeedfollowers_
	key: "buzzfeedfollowers"
	...
	function start(buzzer) {
		//
		eventsThread_ = buzzerClient.createBuzzerFollowersList();
	...
	function closePage() {
		stopPage();
		controller.popPage(buzzfeedfollowers_);
		destroy(1000);
	}
	...
	BuzzerCommands.LoadFollowersByBuzzerCommand {
		id: modelLoader
		model: eventsThread_
	...
	QuarkListView {
		...
		delegate: Item {
			...
			Component.onCompleted: {
				...
				buzzItem.width = list.width;
				buzzItem.controller_ = buzzfeedfollowers_.controller;
