...
#ifndef QBIT_UNITTEST_STORE_H
#define QBIT_UNITTEST_STORE_H
...
class StoreQbitCreateSpend: public Unit {
public:
	StoreQbitCreateSpend(): Unit("StoreQbitCreateSpend") {
		settings_ = std::make_shared<SettingsA>();
		consensus_ = std::make_shared<ConsensusA>();

		store_ = TransactionStore::instance(settings_, nullptr);
