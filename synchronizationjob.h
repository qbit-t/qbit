...
public:
	SynchronizationJob(ISettingsPtr settings, const uint256& chain, const uint256& block, uint64_t delta, Type type) :
		...
		tempspaceBlocks_(strprintf("%s/%s/b-%d", settings->dataPath(), chain.toHex(), timestamp_)),
		tempspaceChunks_(strprintf("%s/%s/c-%d", settings->dataPath(), chain.toHex(), timestamp_)),
